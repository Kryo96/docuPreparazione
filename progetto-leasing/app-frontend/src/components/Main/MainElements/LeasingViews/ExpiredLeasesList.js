...
function ExpiredLeasesList() {
    ...
    useEffect(() => {
        const fetchLeases = async () => {
            try {
                const response = await fetch("http://localhost:8080/contracts/expired");
    ...
    const filteredLeases = leases.filter((lease) => {
        return (
            ...
            (!filters.dataScadenza || lease.dataScadenza.includes(filters.dataScadenza)) &&
    ...
    return (
        <div className="container mt-4">
            <h2 className="mb-4">Contratti di Leasing Scaduti</h2>
            ...
            <div className="mb-4">
                ...
                <input
                    type="date"
                    className="form-control mb-2"
                    placeholder="Filtra per Data Scadenza"
                    name="dataScadenza"
                    value={filters.dataScadenza}
                    onChange={handleFilterChange}
                />
                <input
                    type="number"
...
export default ExpiredLeasesList;
